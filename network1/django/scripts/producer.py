...
cursor.execute("""
    CREATE SEQUENCE IF NOT EXISTS odd1_id_seq START 1 INCREMENT 2;    
    ALTER TABLE certificates
    ALTER COLUMN id SET DEFAULT nextval('odd1_id_seq');
...
def handle_loop():
    def handle_notify():
        pika_connection = pika.BlockingConnection(pika.ConnectionParameters(host='172.17.0.5', port=5672))
        ...
        for notify in postgres_connection.notifies:
            ...
            print('*************************************')
            channel.basic_publish(exchange='replication', routing_key='certificate_django_queue1', body=notify.payload)
...
print('[*] Waiting for database changes to be published. To exit press CTRL+C')
